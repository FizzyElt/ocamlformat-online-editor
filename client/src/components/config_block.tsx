...
const ConfigBlock = ({ config }: ConfigBlockProps): React.ReactNode => {
    const configPair = selectList
        .map((item) => [item.label, config[item.label]])
        .filter(([_, value]) => value !== "")
        .map(([key, value]) => [key.replaceAll("_", "-"), value.replaceAll("_", "-")]);

    const configContent = configPair.map(([key, value]) => `${key}=${value}`).join("\n");
