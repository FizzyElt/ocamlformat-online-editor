import {
    Box,
    Button,
    CloseButton,
    Dialog,
    Flex,
    Grid,
    GridItem,
    HStack,
    Link,
    Portal,
    Spacer,
    Text,
    useDialog,
} from "@chakra-ui/react";
import { useState } from "react";

import ConfigBlock from "./components/config_block";
import { Toaster } from "./components/ui/toaster";
import ConfigForm from "./ConfigForm";
import EditorContent from "./EditorContent";
import { type Config, defaultConfig } from "./type";

function App(): React.ReactNode {
    const [config, setConfig] = useState<Config>(defaultConfig);

    const dialog = useDialog();

    return (
        <>
            <Box w="100vw" h="100vh">
                <Grid
                    gap={4}
                    p={5}
                    w="full"
                    h="full"
                    templateColumns="repeat(2, 1fr)"
                    templateRows="repeat(2, 1fr)"
                >
                    <GridItem colSpan={1} rowSpan={2} overflowY="scroll">
                        <Box minH="full">
                            <Flex
                                mb={4}
                                px={4}
                                pos="sticky"
                                top={0}
                                bg="white"
                                zIndex="sticky"
                                alignItems="center"
                                gap={4}
                            >
                                <Link
                                    variant="underline"
                                    outline="none"
                                    href="https://github.com/FizzyElt/ocamlformat-online-editor"
                                    target="_blank"
                                >
                                    Github
                                </Link>
                                <Text color="gray.600">ocamlformat 0.29.0</Text>
                                <Spacer />
                                <HStack>
                                    <Button
                                        size="lg"
                                        colorPalette="teal"
                                        onClick={() => dialog.setOpen(true)}
                                    >
                                        config
                                    </Button>
                                </HStack>
                            </Flex>
                            <ConfigForm config={config} onChange={setConfig} />
                        </Box>
                    </GridItem>
                    <GridItem colSpan={1} rowSpan={2}>
                        <Box h="full">
                            <EditorContent config={config} />
                        </Box>
                    </GridItem>
                </Grid>
            </Box>
            <Dialog.RootProvider value={dialog} placement="center" scrollBehavior="inside">
                <Portal>
                    <Dialog.Backdrop />
                    <Dialog.Positioner>
                        <Dialog.Content>
                            <Dialog.CloseTrigger asChild>
                                <CloseButton size="sm" />
                            </Dialog.CloseTrigger>
                            <Dialog.Header>
                                <Dialog.Title>Config file</Dialog.Title>
                            </Dialog.Header>
                            <Dialog.Body>
                                <ConfigBlock config={config} />
                            </Dialog.Body>
                        </Dialog.Content>
                    </Dialog.Positioner>
                </Portal>
            </Dialog.RootProvider>
            <Toaster />
        </>
    );
}

export default App;
